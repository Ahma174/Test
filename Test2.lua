...
do
	...
	function utils:DynamicParent(inst: Instance)
		inst.Parent = self:IsThirdParty() and game:GetService("CoreGui") or game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
	end
...
do
	...
	local _writeuifile = isThirdParty and writefile;
	...
	local function saveSettings()
		if isThirdParty then
			writefile("settings.json", httpService:JSONEncode(utils:DeepCopy(customSettings)));
		end
	end
	...
	if isThirdParty and isfile("settings.json") then
		local succ, res = pcall(httpService.JSONDecode, httpService, readfile("settings.json"));
		if succ then
...
do
	...
	local _isuifile = isThirdParty and isfile;
		...
		if self.cache == nil then
			...
			if isThirdParty then
				if _isuifile("cache.json") then
					local success, res = pcall(httpService.JSONDecode, httpService, readfile("cache.json"));
...
do
	...
	local isThirdParty = utils:IsThirdParty();
	globals.customAssetPath = isThirdParty and (function()
		if not isfolder("HydrogenAssets") then makefolder("HydrogenAssets"); end
		return "HydrogenAssets";
	end)() or "";
	...
	local function successfulLogin(directory: Instance)
		basis:Destroy();
		if isThirdParty then
			for i, v in { "IjHyfuyuHeg", "cHjGyjKbe", "cHpRmejNAJ" } do
				getgenv()[v] = nil;
			end
			if globals.customSettings.autoExecute then
				if runautoexec then runautoexec(); end
			end
		end
	...
	local function loadLoginPage()
		...
		login.adless.loginBtn.MouseButton1Click:Connect(function()
			-- adless login unavailable (native function not recoverable from dump)
		end);
	...
	local function setupBootstrapper()
		if isThirdParty then
			for i, v in assets.fonts do
				...
				if not isfolder("Custom Fonts") then makefolder("Custom Fonts"); end
				if not isfolder(string.format("Custom Fonts/%s", i)) then makefolder(string.format("Custom Fonts/%s", i)); end
				for _, v2 in v do
					local fileName = string.format("Custom Fonts/%s/%s-%s.ttf", i, i, v2);
					if isfile(fileName) == false then
						writefile(fileName, game:HttpGet(string.format("https://raw.githubusercontent.com/VersatileTeam/hydrogen-assets/main/fonts/%s/%s-%s.ttf", i, i, v2), true));
				...
				if hasDownloadedSomething then
					...
					writefile(string.format("Custom Fonts/%s/%s.json", i, i), string.gsub(fontData, "Custom Fonts", globals.customAssetPath .. "/Custom Fonts"));
				end
				updateCount();
			end
	...
	function tabSystem:Initialize(basis: Frame)
		...
		if isThirdParty then
			if _isuifile("tabs.json") then
				local success, res = pcall(httpService.JSONDecode, httpService, readfile("tabs.json"));
...
do
	...
	local _writecustomasset = utils:IsThirdParty() and writefile;
			...
			if string.sub(scriptResult.game.imageUrl, 1, 4) == "http" then
				...
				elseif scriptResult.game.name ~= "Universal Script 📌" then
					...
					if placeIdCache[gameId] == nil then
						...
						if res then
							placeIdCache[gameId] = httpService:JSONDecode(res).data[1].imageUrl;
						end
					end
					x.background.Image = placeIdCache[gameId] or "";
					return;
				else
					return;
				end
			end
			x.background.Image = scriptResult.game.imageUrl or "";
		end);
